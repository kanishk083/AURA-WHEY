...
const assets = {
  hero: [
    ['./assets/optimized/hero-combo-1920.jpg', './assets/optimized/hero-combo-960.jpg'],
    ['./assets/optimized/hero-chocolate-1920.jpg', './assets/optimized/hero-chocolate-960.jpg'],
    ['./assets/optimized/hero-kulfi-1920.jpg', './assets/optimized/hero-kulfi-960.jpg'],
    ['./assets/optimized/hero-benefits-1920.jpg', './assets/optimized/hero-benefits-960.jpg'],
    ['./assets/optimized/hero-routine-1920.jpg', './assets/optimized/hero-routine-960.jpg']
  ],
...
const productFlavours = {
  'Mawa Kulfi': {
    theme: 'flavour-kulfi',
    images: [1, 2, 3, 4, 5].map(index => `./assets/optimized/kulfi-${index}.jpg`)
  },
  'Rich Chocolate': {
    theme: 'flavour-chocolate',
    images: [1, 2, 3, 4, 5].map(index => `./assets/optimized/chocolate-${index}.jpg`)
  }
};
...
const heroSlides = [
  {
    ...
    desktopImage: assets.hero[0][0],
    mobileImage: assets.hero[0][1],
    link: '#/shop'
  },
  {
    id: 'rich-chocolate',
    ...
    desktopImage: assets.hero[1][0],
    mobileImage: assets.hero[1][1],
    link: '#/shop'
  },
  {
    id: 'mawa-kulfi',
    ...
    desktopImage: assets.hero[2][0],
    mobileImage: assets.hero[2][1],
    link: '#/shop'
  },
  {
    ...
    desktopImage: assets.hero[3][0],
    mobileImage: assets.hero[3][1],
    link: '#/shop'
  },
  {
    ...
    desktopImage: assets.hero[4][0],
    mobileImage: assets.hero[4][1],
    link: '#/shop'
  }
];

const state = {
  flavour: 'Mawa Kulfi', productImage: 0, cartItems: {}, tab: 'Details', coupon: '',
...
const image = (src, alt, className = '') => `<img class="${className}" src="${src}" alt="${alt}" loading="lazy" decoding="async" />`;
        ...
        <div class="header">
          <div class="header-inner">
            ...
            <div class="header-actions">
              ...
              ${routeLink('cart', `${icon('bag')}<span class="cart-count" aria-label="${cartQuantity} items in cart">${cartQuantity}</span><span class="sr-only">Cart</span>`, 'icon-button cart-link')}
              ...
              <button class="icon-button menu-button" type="button" data-action="open-menu" aria-label="Open menu" aria-controls="mobile-menu" aria-expanded="false">${icon('menu')}</button>
            </div>
          </div>
        </div>
      </header>
      <button class="overlay" type="button" data-action="close-menu" tabindex="-1" aria-hidden="true" aria-label="Close menu"></button>
...
function heroImageForViewport(slide) {
  ...
  return isMobile && slide.mobileImage ? slide.mobileImage : slide.desktopImage;
}

function syncHeroAmbientImage() {
  const ambient = document.querySelector('#hero-ambient-bg');
  const slide = heroSlides[state.heroSlide];
  if (ambient && slide) ambient.style.backgroundImage = `url('${heroImageForViewport(slide)}')`;
}
...
function heroBannerCarousel() {
  ...
  return `
    <div class="hero-section-wrap">
      <div class="hero-ambient-bg" id="hero-ambient-bg" style="background-image: url('${heroImageForViewport(currentSlide)}');"></div>
              ...
              <div class="hero-slide ${isActive ? 'is-active' : ''}" data-slide-index="${index}" role="group" aria-roledescription="slide" aria-label="${slide.title}" aria-hidden="${!isActive}">
                <a href="${slide.link || '#/shop'}" class="hero-slide-link" tabindex="${isActive ? '0' : '-1'}">
                  <picture class="hero-picture">
                    ...
                    <img class="hero-banner-img" src="${slide.desktopImage}" alt="${slide.alt || slide.title}" width="1920" height="730" sizes="100vw" loading="${index === 0 ? 'eager' : 'lazy'}" decoding="async" fetchpriority="${index === 0 ? 'high' : 'low'}" />
                  </picture>
                </a>
              </div>`;
          }).join('')}
...
function home() {
  return `
    ...
    <section class="section"><div class="section-inner"><div class="section-head"><div><h2>Before you order</h2><div class="gold-rule"></div></div><p>The quick answers customers look for most.</p></div>${faqItems(6)}<p>${routeLink('faq', 'Read all FAQs', 'button-link secondary')}</p></div></section>`;
}
...
function addToCart(flavour) {
  state.cartItems[flavour] = (state.cartItems[flavour] || 0) + 1;
}

function totals() {
  const subtotal = 4199 * cartCount();
...
const faqs = [
  ...
  ['How should I prepare my shake?', 'Follow the mixing directions printed on your pack. You can adjust the amount of water or milk to suit your preferred consistency; remember that milk and other additions change the shake’s nutrition.'],
  ['When is the best time to take whey protein?', 'Whey protein can be used whenever it conveniently helps you meet your daily protein needs, including around a workout or between meals. Your overall diet and consistent routine matter more than a single fixed time.'],
  ['How many servings should I take each day?', 'Protein needs vary with body size, diet, activity, and goals. Follow the serving guidance on the pack and count Aura Whey as part of your total daily protein intake. Ask a qualified dietitian or healthcare professional if you need personalised advice.'],
  ['Does Aura Whey contain milk or other allergens?', 'Whey is derived from milk. Check the ingredients and allergen statement on your specific pack before use because formulations and flavours may differ. If you have an allergy or intolerance, seek qualified medical advice before using the product.'],
  ['Can I add Aura Whey to smoothies or food?', 'You can mix whey protein into a shake, smoothie, oats, or another suitable recipe. Follow the pack directions and account for any additional ingredients in your overall nutrition.'],
  ['How should I store the product?', 'Keep the container tightly closed and follow the storage instructions and best-before information printed on the pack. Avoid moisture, heat, and direct sunlight.'],
  ...
  ['What should I do if my order arrives damaged?', 'Keep the packaging and take clear photos of the parcel and product, then contact support with your order number as soon as possible so the issue can be reviewed.']
];

function faqItems(limit = faqs.length) { return `<div class="accordion">${faqs.slice(0, limit).map(([question, answer], index) => `<div><button type="button" data-action="faq-${index}" aria-expanded="false"><span>${question}</span>${icon('chevron')}</button><div class="accordion-panel" hidden>${answer}</div></div>`).join('')}</div>`; }
...
function setHeroSlide(index) {
  ...
  if (ambientBg && heroSlides[state.heroSlide]) {
    ambientBg.style.backgroundImage = `url('${heroImageForViewport(heroSlides[state.heroSlide])}')`;
  }
...
function openMobileMenu() {
  ...
  setTimeout(() => panel?.querySelector('.menu-close')?.focus(), 0);
}
...
function handleMenuKeydown(event) {
  ...
  const focusable = [...panel.querySelectorAll('a[href], button:not([disabled]), input:not([disabled]), textarea:not([disabled]), [tabindex]:not([tabindex="-1"])')];
...
function bindEvents() {
  document.querySelectorAll('[data-route]').forEach(link => link.addEventListener('click', () => { closeMobileMenu(false); setTimeout(() => document.querySelector('main')?.focus(), 0); }));
...
function handleAction(action, element) {
  ...
  if (action === 'open-menu') { openMobileMenu(); return; }
  if (action === 'close-menu') { closeMobileMenu(); return; }
  ...
  if (action.startsWith('add-flavour-')) { state.flavour = action.replace('add-flavour-', ''); state.productImage = 0; addToCart(state.flavour); return navigate('cart'); }
  if (action.startsWith('tab-')) { state.tab = action.replace('tab-', ''); return render(); }
  if (action === 'add-cart') { addToCart(state.flavour); return navigate('cart'); }
...
window.addEventListener('resize', syncHeroAmbientImage, { passive: true });
